...
Aria2 Downloader 是一个 Clipbus 插件，用于从剪贴板条目中识别可下载链接，并将用户选中的资源通过 aria2 JSON-RPC 提交为下载任务。

## ✨ 功能

- 🔎 识别 `text` 剪贴板条目中的下载链接。
- 📎 识别 `path_reference` 剪贴板条目中的本地 torrent 和 metalink 文件。
- 🧩 在 Clipbus 中显示紧凑的 attachment renderer。
- ✅ 支持多个识别结果，并允许提交前逐项选择或排除。
- 🚀 通过 aria2 JSON-RPC 提交选中的下载资源。
- 🎨 使用 Clipbus 主题色渲染插件界面，并使用蓝色 attachment 强调色。
- 🆘 提供原生 `Help` action，用于打开项目页面。
- ⚙️ 从 Clipbus external settings 读取 aria2 RPC 配置；缺少必要配置时禁用提交。
...
然后在 Clipbus external settings 中配置对应值。
...
- 详细提交失败原因也会通过 Clipbus plugin console 记录。
...
dev 页面只用于本地 UI 迭代。最终展示效果仍需在 Clipbus 中重新构建并 Reload 插件后验证。

## 📦 Clipbus 安装

本地开发时，将插件根目录添加到 Clipbus Developer Plugins。

Clipbus 会加载 `manifest.json` 中声明的构建产物：

- Runtime：`dist/plugin.cjs`
- UI 根目录：`dist/ui`

修改 `manifest.json`、detector ID、attachment type、runtime 代码或 UI 入口后，需要重新构建并在 Clipbus 中 Reload 插件。

## 🏷️ 插件标识

- 插件 ID：`plugin.clipbus.aria2`
